...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch(`http://127.0.0.1:5000/api/v1/players/${player1Name}`)
        .then(response => response.json())
        .then(player1 => {
            fetch(`http://127.0.0.1:5000/api/v1/players/${player2Name}`)
                .then(response => response.json())
                .then(player2 => {
    ...
    function fetchAndDisplayChart(selectedField, player1Name, player2Name) {
        ...
        fetch(`http://127.0.0.1:5000/api/v1/players/${player1Name}/${selectedField}`)
            .then(response => response.json())
            .then(dataPlayer1 => {
                fetch(`http://127.0.0.1:5000/api/v1/players/${player2Name}/${selectedField}`)
                    .then(response => response.json())
                    .then(dataPlayer2 => {
